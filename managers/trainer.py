...
class Trainer():
    ...
    def train_epoch(self):
        ...
        for b_idx, batch in bar:
            ...
            # BCELoss
            # m = nn.Sigmoid()
            score_pos = torch.squeeze(score_pos)


            # score_pos = m(score_pos)


            # Ensure score_pos and r_labels_pos have the same size
            if score_pos.dim() == 0:
                score_pos = score_pos.view(1)
            if score_pos.shape != r_labels_pos.shape:
                score_pos = score_pos.view(r_labels_pos.shape,-1)

            # print("score_pos shape:", score_pos.shape)
            # print("r_labels_pos shape:", r_labels_pos.shape)
            ...
            with torch.no_grad():
                ...
                print("batch_auc",train_auc)
                print("batch_aupr", train_aupr)
